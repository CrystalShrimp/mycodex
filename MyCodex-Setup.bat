...
if exist "%MSI_PATH%" (
    ...
    if errorlevel 1 (
        ...
        echo [X] Node.js 静默安装失败（通常是因为没有管理员权限）。
...
call npm install
if errorlevel 1 (
    echo [X] npm install 失败，请检查网络后重跑本脚本。
    popd
    goto CHECK_ENV
)
...
if errorlevel 1 (
    echo [X] Playwright Chromium 安装失败，请检查网络后重跑本脚本。
    popd
    goto CHECK_ENV
)
popd
...
(
    ...
    echo.
    echo # ===== Agent Config =====
    echo # Codex auth uses the local ChatGPT login (~/.codex/auth.json), no API key needed.
